...
class Allow21341234123412 {
	...
	*/
	...
	public static function valid_action_permission($module_name,$perimission_name){
        ...
        /**
         * @todo Полные права на действия, т.к. новые имена модулей не совпадают со старыми ролями ( news != admin_news )
         */
        return true;
	...
	*/
	...
	public static function enabled_module($module_name){
		...
		if(Modules::where('name', $module_name)->exists() && Modules::where('name', $module_name)->first()->on == 1):
			return TRUE;
		elseif(!Modules::where('name', $module_name)->exists() && SystemModules::getModules($module_name,3)):
			return TRUE;
		else:
			return FALSE;
		endif;
