			...
			@foreach($users as $user)
				...
				@if(Auth::user()->id != $user->id)
					@if(Allow::action('users', 'edit'))
						<a class="btn btn-labeled btn-success pull-left margin-right-10" href="{{ link::auth('users/edit/'.$user->id) }}">
							<span class="btn-label"><i class="fa fa-edit"></i></span> Ред.
						</a>
					@endif
					@if(Allow::action('users', 'delete'))
						<form method="POST" action="{{ link::auth('users/destroy/'.$user->id) }}">
...
<script>
	$('.group-add-select').on('change', function(){
		...
		{
			$.ajax({
				url: '{{ link::to('admin/users/attach') }}',
				data: { group_id: $_select.val(), user_id: $_select.data('user') },
				type: 'post'
