...
<div class="row">
	<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
		...
		@if(Allow::action('users','create'))
			<!--<a class="btn btn-primary" href="{{ link::auth('groups/create')}}">Добавить группу</a>-->
		@endif
		</div>
	</div>
</div>
		...
		<table class="table table-striped table-bordered">
			...
			@foreach($groups as $group)
				<tr>
					...
					<td>{{ $group->desc }}</td>
					<td>
						<a class="btn btn-labeled btn-success pull-left margin-right-10" href="{{ link::auth('groups/edit/'.$group->id)}}">
...
<script>
	...
	$('.roles-checkbox').click(function(){
		...
		if($box.is(':checked')){
			ajaxGroupRole('{{ link::auth('groups/attach') }}', $box);
		} else {
			ajaxGroupRole('{{ link::auth('groups/detach') }}', $box);
		}
	});
</script>
@stop
