...
<aside id="left-panel">
	<nav>
    	...
    	@foreach(SystemModules::getSidebarModules() as $name => $module)
    		@if( (Module::where('name', $module['name'])->exists() && Module::where('name', $module['name'])->first()->on == 1) || @$module['system'] == 1 )
<? #echo (string)Request::segment(2) . " == " . $module['link'] . " > " . (int)((string)Request::segment(2) == $module['link']); ?>
<? #dd(Request::segment(2)); ?>
			<li{{ ((string)Request::segment(2) == $module['link']) ? ' class="active open"' : '' }}>
			    ...
			    @if(isset($module['menu_child']) && !empty($module['menu_child']))
